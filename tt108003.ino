...
#if defined(NOK5110) || defined(OLED096)
  oled.print("Transistor Tester");
  oled.setCursor(0, 1);
  oled.print("for Arduino");
  oled.setCursor(0, 5);
  oled.print("v. 1.08.003");
  oled.setCursor(0, 6);
  oled.print("OLED mod by Xiorcid");
...
#define T2_PERIOD (1024 / (F_CPU / 1000000UL));	// set to 128 or 64 us 
  ...
  if (tmp) {
    ...
    lcd_line1();
    lcd_fix_string("Timeout!");	// Output Timeout
...
#define POOR_LEVEL (((unsigned long)(BAT_POOR)*(unsigned long)33)/133)
  ...
  if (trans.uBE[0] <  WARN_LEVEL) {
    ...
    if (trans.uBE[0] < POOR_LEVEL) {
      ...
      oled.print("Battery empty!");	// Battery empty!
      wait_about2s();
      PORTD = 0;			// switch power off
      return;
    }

    oled.print("Battety weak");		// Battery weak
  } else {                            // Battery-voltage OK
    oled.print("OK"); 		// "OK"
  }

#else
  oled.print("v. 1.08.003 mod");	// if no Battery check, Version .. in row 1
#endif
  ...
  oled.setCursor(0, 2);			// LCD position row 2, column 1

  oled.print("Testing...");	// String: testing...
  ...
  if (PartFound == PART_DIODE) {
    if (NumOfDiodes == 1) {		// single Diode
      ...
      oled.drawBitmap(0, 0, DIODE_60x60, 60, 60);
      oled.update();
      ...
      if (diodes[0].Anode > diodes[0].Cathode) {
        oled.setCursor(64, 0);
        oled.print("A= ");
        oled.print(diodes[0].Anode);
        oled.setCursor(64, 1);
        oled.print("C= ");
        oled.print(diodes[0].Cathode);
      } else {
        oled.setCursor(64, 0);
        oled.print("A= ");
        oled.print(diodes[0].Anode);
    ...
    if (PartMode == PART_MODE_NPN) {
      oled.drawBitmap(0, 0, NPN_60x60, 60, 60);
      oled.update();
     // lcd_fix_string(NPN_str);		// "NPN "
    } else {
      oled.drawBitmap(0, 0, PNP_60x60, 60, 60);
      oled.update();
    ...
    if (PartMode & 1) {
      oled.print('P');			// P-channel
    } else {
      oled.print('N');			// N-channel
    }
    oled.print('-');
    ...
    oled.drawBitmap(0, 0, CAP_60x60, 60, 60);
    oled.update();
    oled.setCursor(64, 0);
    oled.print("A=");
    oled.print(cap.ca);		// Pin number 1
    oled.setCursor(64, 1);
    oled.print("B=");
    oled.print(cap.cb);		// Pin number 2
    ...
    if (cap.v_loss != 0) {
      ...
      oled.print("Vloss=");	// "  Vloss="
      DisplayValue(cap.v_loss, -1, '%', 2);
    }
#endif

    oled.setCursor(64, 2);  // 2 row
    oled.print("Cap=");
    Serial.println(cap.cval_max);
    Serial.println(cap.cpre_max);
    ...
    if (cap.esr < 65530) {
      ...
      oled.print("ESR=");
      Serial.println(cap.esr);
      DisplayValue(cap.esr, -2, "Ohm", 4);
    }
#endif
  ...
  if (NumOfDiodes == 0) { 		// no diodes are found
    oled.print("No, unknown, or"); 	// "No, unknown, or"
    oled.setCursor(0, 2); 			// 2 row
    oled.print("damaged part"); 	// "damaged "
  } else {
    lcd_fix_string("part");		// "part"
    lcd_fix_string(" unknown"); 		// " unknown"
    lcd_line2(); 			// 2 row
    lcd_fix_string("or damaged "); 		// "or damaged "
...
void mVOutput(uint8_t nn) {
  if (nn < 3) {
    // Output in mV units
    DisplayValue(diodes[nn].Voltage, -3, 'V', 2);
    oled.print(" ");
  }
}
...
void DisplayValue(unsigned long Value, int8_t Exponent, unsigned char Unit, unsigned char digits)
{
  ...
  if (Exponent <= 0)			// we have to prepend "0."
  {
    ...
    while (Exponent < 0)
    {
      oled.print('0');		// extra 0 for factor 10
      Exponent++;
    }
#else
    if (Exponent < 0) oled.print('0');	// extra 0 for factor 100
#endif
  }
  ...
  {
    oled.print(String(OutBuffer[Index]));		// display char
    Index++;				// next one
    if (Index == Exponent) {
      oled.print('.');			// display dot
    }
  }

  // display prefix and unit
  if (Prefix != 0) oled.print(Prefix);
...
void PinLayout(char pin1, char pin2, char pin3) {
  ...
  oled.print("123=");			// " 123="
  for (ii = 0; ii < 3; ii++) {
    if (ii == trans.e)  oled.print(pin1);	// Output Character in right order
    if (ii == trans.b)  oled.print(pin2);
    if (ii == trans.c)  oled,print(pin3);
  }
#else
#if EBC_STYLE == 321
  // Layout with 321= style
